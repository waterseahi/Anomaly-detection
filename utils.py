...
def get_data(dataset, max_train_size=None, max_test_size=None,
    ...
    f = open(( "datasets/EPC/processed/rainepc_train.pkl"), "rb")
    train_data = pickle.load(f).reshape((-1, x_dim))[train_start:train_end, :]
    f.close()
    try:
        f = open(( "datasets/EPC/processed/estepc_test.pkl"), "rb")
        test_data = pickle.load(f).reshape((-1, x_dim))[test_start:test_end, :]
        f.close()
    except (KeyError, FileNotFoundError):
        test_data = None
    try:
        f = open(( "datasets/EPC/processed/estlabelepc_test_label.pkl"), "rb")
